  ...
  if length Sys.argv < 2 then begin
    ...
    match Sys.argv.(1) with
      "init" -> 
        ...
        then begin 
          ...
          Unix.mkdir ".confident/default/conf" 0o755;
          Unix.mkdir ".confident/default/result" 0o755;
          ...
          in output_string oc ( "This is the default profile\n" ); close_out oc;
    ...
    | "list" | "ls" -> 
      ...
      then begin 
        ...
        let box_name = get_current_box () in
        Array.iter 
    ...
    | "set" ->
      ( match length Sys.argv with
        2 | 3 -> print_string "[Error] insufficient arguments"; print_newline ()
      | _ -> let conf_file = ( "conf/" ^ Sys.argv.(2) ) and assignment = Sys.argv.(3) in
             let oc = open_out ".confident/tmp" in output_string oc assignment; close_out oc;
             let updates = parse ".confident/tmp" and original = parse conf_file
             in save_conf_list conf_file (update_variables original updates); 
             output_conf_list stdout updates;
             print_string ( "[ok] " ^ conf_file ^ " gets updated." );
             print_newline ()
      )
